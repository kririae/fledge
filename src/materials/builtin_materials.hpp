...
struct BeckmannDistribution {
  F_CPU_GPU static Float roughnessToAlpha(Float roughness) {
  ...
  F_CPU_GPU BeckmannDistribution(Float alpha)
      : m_alpha_x(alpha), m_alpha_y(alpha) {}
  F_CPU_GPU BeckmannDistribution(Float alpha_x, Float alpha_y)
      : m_alpha_x(alpha_x), m_alpha_y(alpha_y) {}
  F_CPU_GPU Float D(const Vector3f &wh) const {
  ...
  F_CPU_GPU Float lambda(const Vector3f &w) const {
  ...
  F_CPU_GPU Float G1(const Vector3f &w) const { return 1 / (1 + lambda(w)); }
  F_CPU_GPU Float G(const Vector3f &wo, const Vector3f &wi) const {
    return 1 / (1 + lambda(wo) + lambda(wi));
  }
  F_CPU_GPU Float pdf([[maybe_unused]] const Vector3f &wo,
                      const Vector3f                  &wh) const {
    return D(wh) * AbsCosTheta(wh);
  }
  F_CPU_GPU Vector3f sampleWh(const Vector3f &wo, const Vector2f &u) const {
